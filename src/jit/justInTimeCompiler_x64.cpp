	...
	std::vector<ds::RuntimeFunction>& vTable)
{
	...
	while (!code.empty())
	{
		...
		case ds::BytecodeOp::greaterInt: {
			...
			int32_t stackPosition = -(sizeof(Int) + argument.size);
			...
			{
				assembler->cmp(argument.gpRegister, ptr_32(stackRegister, stackPosition));
			}
			auto successLabel = assembler->new_label();
			auto failLabel = assembler->new_label();

			assembler->setl(al);
			changeStackBy(-(sizeof(Int) + argument.size));
			compilePushValue(al);
			break;
		}
